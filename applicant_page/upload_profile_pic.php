...
if (isset($_FILES['profile_pic_file']) && $_FILES['profile_pic_file']['error'] === UPLOAD_ERR_OK) {
    ...
    $stmt_old = $conn->prepare("SELECT profile_pic FROM applicants WHERE id = ?");
    $stmt_old->bind_param("i", $applicant_id);
    $stmt_old->execute();
    $old_pic_path = $stmt_old->get_result()->fetch_assoc()['profile_pic'] ?? null;
    $stmt_old->close();

    $new_filename = 'pfp_' . $applicant_id . '_' . time() . '.' . $file_extension;
    $target_path = $upload_dir . $new_filename;

    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        $stmt_update = $conn->prepare("UPDATE applicants SET profile_pic = ? WHERE id = ?");
